...
export default function RelatoriosPage() {
  ...
  const exportarPDF = () => {
    ...
    // Título
    doc.setFontSize(18)
    doc.text("SIGA - Relatório Geral", 14, 20)
    ...
    doc.setFontSize(10)
    doc.text(`Total de Alunos: ${alunos.length}`, 14, 48)
    doc.text(`Total de Turmas: ${turmas.length}`, 14, 54)
    doc.text(`Média Geral: ${mediaGeral.toFixed(1)}`, 14, 60)
    doc.text(`Taxa de Aprovação: ${((alunosAprovados / alunos.length) * 100).toFixed(1)}%`, 14, 66)
    
    // Desempenho Geral
    doc.setFontSize(14)
    doc.text("Desempenho Geral", 14, 80)
    
    const desempenhoData = [
      ["Situação", "Quantidade", "Percentual"],
      ["Aprovados (≥ 7.0)", alunosAprovados.toString(), `${((alunosAprovados / alunos.length) * 100).toFixed(1)}%`],
      ["Recuperação (5.0 - 6.9)", alunosRecuperacao.toString(), `${((alunosRecuperacao / alunos.length) * 100).toFixed(1)}%`],
      ["Reprovados (< 5.0)", alunosReprovados.toString(), `${((alunosReprovados / alunos.length) * 100).toFixed(1)}%`],
    ]
    
    autoTable(doc, {
      startY: 85,
      head: [desempenhoData[0]],
      body: desempenhoData.slice(1),
      theme: 'grid',
    })
    ...
    doc.addPage()
    doc.setFontSize(14)
    doc.text("Desempenho por Turma", 14, 20)
    
    const turmasData = [
      ["Turma", "Código", "Total", "Aprovados", "Recuperação", "Reprovados", "Média"],
      ...relatoriosPorTurma.map(t => [
        t.nome,
        t.codigo,
        t.totalAlunos.toString(),
        t.aprovados.toString(),
        t.recuperacao.toString(),
        t.reprovados.toString(),
        t.media.toFixed(1)
      ])
    ]
  ...
  const exportarExcel = () => {
    ...
    const estatisticas = [
      ["SIGA - Relatório Geral"],
      [`Data: ${new Date().toLocaleDateString('pt-BR')}`],
      [],
      ["Estatísticas Gerais"],
      ["Total de Alunos", alunos.length],
      ["Total de Turmas", turmas.length],
      ["Média Geral", parseFloat(mediaGeral.toFixed(1))],
      ["Taxa de Aprovação", `${((alunosAprovados / alunos.length) * 100).toFixed(1)}%`],
      [],
      ["Desempenho Geral"],
      ["Situação", "Quantidade", "Percentual"],
      ["Aprovados (≥ 7.0)", alunosAprovados, `${((alunosAprovados / alunos.length) * 100).toFixed(1)}%`],
      ["Recuperação (5.0 - 6.9)", alunosRecuperacao, `${((alunosRecuperacao / alunos.length) * 100).toFixed(1)}%`],
      ["Reprovados (< 5.0)", alunosReprovados, `${((alunosReprovados / alunos.length) * 100).toFixed(1)}%`],
    ]

    // Planilha 2: Desempenho por Turma
    const turmasExcel = [
      ["Desempenho por Turma"],
      [],
      ["Turma", "Código", "Total Alunos", "Aprovados", "Recuperação", "Reprovados", "Média", "Taxa Aprovação"],
      ...relatoriosPorTurma.map(t => [
        t.nome,
        t.codigo,
        t.totalAlunos,
        ...
        t.recuperacao,
        t.reprovados,
        parseFloat(t.media.toFixed(1)),
        `${((t.aprovados / t.totalAlunos) * 100).toFixed(1)}%`
      ])
    ]

    // Planilha 3: Lista de Alunos
    const alunosExcel = [
      ["Lista de Alunos"],
      [],
      ["Nome", "Turma", "Média", "Situação"],
      ...alunos.map(a => [
        a.nome,
        a.turma,
        parseFloat(a.media.toFixed(1)),
        a.media >= 7 ? "Aprovado" : a.media >= 5 ? "Recuperação" : "Reprovado"
      ])
    ]
    ...
    const ws1 = XLSX.utils.aoa_to_sheet(estatisticas)
    const ws2 = XLSX.utils.aoa_to_sheet(turmasExcel)
    const ws3 = XLSX.utils.aoa_to_sheet(alunosExcel)
    
    XLSX.utils.book_append_sheet(wb, ws1, "Estatísticas")
    XLSX.utils.book_append_sheet(wb, ws2, "Por Turma")
    XLSX.utils.book_append_sheet(wb, ws3, "Alunos")
    
    XLSX.writeFile(wb, "relatorio-geral.xlsx")
  }

  const exportarRelatorioEspecifico = (tipo: string) => {
    const doc = new jsPDF()
    
    doc.setFontSize(18)
    doc.text(`SIGA - ${tipo}`, 14, 20)
    doc.setFontSize(10)
    doc.text(`Data: ${new Date().toLocaleDateString('pt-BR')}`, 14, 28)
    
    if (tipo === "Alunos em Risco") {
      const alunosRisco = alunos.filter(a => a.media < 7)
      
      doc.setFontSize(14)
      doc.text("Alunos em Risco Acadêmico", 14, 40)
      ...
      const data = [
        ...
        ...alunosRisco.map(a => [
          ...
          a.turma,
          a.media.toFixed(1),
          a.media >= 5 ? "Recuperação" : "Reprovado"
        ])
      ]
    ...
    } else if (tipo === "Frequência") {
      doc.setFontSize(14)
      doc.text("Relatório de Frequência", 14, 40)
      doc.text("(Funcionalidade em desenvolvimento)", 14, 50)
    } else if (tipo === "Desempenho Mensal") {
      doc.setFontSize(14)
      doc.text("Desempenho Mensal", 14, 40)
      doc.text("(Funcionalidade em desenvolvimento)", 14, 50)
    }
    
    doc.save(`relatorio-${tipo.toLowerCase().replace(/\s/g, '-')}.pdf`)
  }
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Button variant="outline" onClick={() => exportarRelatorioEspecifico("Alunos em Risco")}>
                <Calendar className="h-4 w-4 mr-2" />
                Alunos em Risco
              </Button>
              <Button variant="outline" onClick={() => exportarRelatorioEspecifico("Frequência")}>
                <Users className="h-4 w-4 mr-2" />
                Relatório de Frequência
              </Button>
              <Button variant="outline" onClick={() => exportarRelatorioEspecifico("Desempenho Mensal")}>
